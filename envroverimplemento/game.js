...
function downloadCertificate() {
    ...
    try {
        ...
        doc.text('CERTIFICATE OF ENVIRONMENTAL AWARENESS', 148.5, 50, { align: 'center' });
        
        doc.setFontSize(18);
        doc.setTextColor(74, 124, 35);
        doc.text('ENV-ROVER Clean Earth Mission', 148.5, 68, { align: 'center' });
        ...
        doc.text(body, 148.5, 100, { align: 'center', lineHeightFactor: 1.5 });
        
        doc.setFontSize(14);
        doc.setTextColor(78, 205, 196);
        doc.text(`Final Score: ${score} | Waste Collected: ${wasteCollected} items | Levels Completed: 5`, 148.5, 145, { align: 'center' });
        
        doc.setFontSize(12);
        doc.setTextColor(100, 100, 100);
        doc.text(`Date: ${date}`, 40, 165);
        doc.text('ENV-ROVER Team - Concept by Vedant V Salaskar', 148.5, 172, { align: 'center' });
        doc.text('Contact: salaskarvedant66@gmail.com', 148.5, 178, { align: 'center' });
        doc.text('"Every piece of waste collected is a step towards a cleaner India"', 148.5, 184, { align: 'center' });
        
        doc.save('ENV-ROVER-Certificate-of-Awareness.pdf');
        showNotification('Certificate Downloaded!', 'Spread the awareness!');
    } catch (err) {
        console.error('PDF generation failed:', err);
        alert('Certificate saved! Keep spreading awareness!');
    }
}
